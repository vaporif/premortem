...
4b. **LLM recall fallback** (always on, predicate-gated). When fewer than `N_synthesize` candidates clear `min_similarity_score` after rerank, an Opus call names candidate comparables from training data. The recall LLM returns candidate names; Tavily search anchors each to a real citation; the verifier proves the citation. URL hallucinations no longer reach the verifier — they fail at the search step or get replaced by a real URL. Surviving candidates persist as `source=llm_recall` corpus entries. The branch fires automatically; `--force-llm-recall` bypasses the predicate (cassette/eval). See `docs/plans/2026-05-08-llm-recall-fallback.md` for the original design, `docs/plans/2026-05-09-recall-always-on.md` for why it shipped opt-in originally, and `docs/plans/2026-05-11-recall-search-then-verify.md` for the search-then-verify cutover.

L0 — search head. Per suggestion, one Tavily search shaped by `status` and `failure_year`; first hit whose title-or-snippet contains the company name plus a death keyword wins (fallback: first name-only hit). Zero name-matching hits drops the suggestion — equivalent to "the article URL would have 404'd anyway, save the L2 round-trip". The verifier no longer gates on the homepage URL — the homepage is provenance, not a citation, and "dead homepage" is exactly the population we're trying to admit. On the discovered citation URL, HEAD failure falls through to GET so paywalled and anti-bot citations don't false-drop on hosts that answer GET but not HEAD. Wayback enrichment is advisory and gated on `homepage_url is not None`: its failure or absence never drops a candidate, because archive.org coverage is patchy enough that "no snapshot" isn't a reliable realness signal; when the recall LLM didn't supply a homepage, L4 short-circuits with no round-trip and `wayback_attempted="false"` on the verified event. `WaybackEnricher.enrich()` is called directly — the enricher already retries 3× internally with exponential backoff and swallows transient errors, so wrapping it in another retry just delays the inevitable. The persisted body always contains the news article (the citation L5 verifies against); when Wayback anchors, its snapshot is prepended under a "Vendor description (archived)" section so synthesis can read both the value-prop and the death narrative from a single entry. Persisted recall entries flow through the standard heading-aware chunker — section markers keep boundaries clean, each chunk inherits the same payload so synthesis still sees the full combined body, and per-chunk vectors give the marketing-copy section a focused signal that matches pitches better than a diluted whole-body vector would. The slop classifier is bypassed for recall entries: L5 is the stricter gate operating on the death-citation substrate, and slop was tuned on a different body shape. L5 is tri-state (`dead` / `struggling` / `alive`); admitted verdicts ride the qdrant payload as `deathness_verdict` and the synthesis prompt reads them, so a struggling vendor produces a forward-looking distress report rather than a post-mortem. After recall persists and re-rerank runs, `compute_coverage_gap` fires a second time and emits `RECALL_GAP_SCORE_AFTER`, so prod telemetry can answer "did recall close the gap on this query" without an offline eval re-run. See `docs/plans/2026-05-10-recall-verifier-hardening.md` for the verifier hardening pass.
